...
// pull in Mongoose model for whiskeys
...
// SHOW
// GET /whiskeys/5a7db6c74d55bc51bdf39793
router.get('/whiskeys/:id', requireToken, (req, res, next) => {
  // req.params.id will be set based on the `:id` in the route
  Whiskey.findById(req.params.id)
    .then(handle404)
    // if `findById` is succesful, respond with 200 and "whiskey" JSON
...
router.post('/whiskeys', requireToken, (req, res, next) => {
  ...
  Whiskey.create(req.body.whiskey)
    // respond to succesful `create` with status 201 and JSON of new "whiskey"
    .then(whiskey => {
...
// UPDATE
// PATCH /whiskeys/5a7db6c74d55bc51bdf39793
...
// DESTROY
// DELETE /whiskeys/5a7db6c74d55bc51bdf39793
router.delete('/whiskeys/:id', requireToken, (req, res, next) => {
  Whiskey.findById(req.params.id)
    .then(handle404)
    .then(whiskey => {
      // throw an error if current user doesn't own `whiskey`
      requireOwnership(req, whiskey)
      // delete the whiskey ONLY IF the above didn't throw
      whiskey.deleteOne()
    })
